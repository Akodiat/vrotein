...
import {
    recongizedProteinResidues,
    recongizedDNAResidues,
    recongizedDNAStrandEnds,
    recongizedRNAResidues,
    recongizedRNAStrandEnds,
    proelem,
    nucelem,
    pdbatom,
    pdbresidue,
...
/*
Protein loading code adapted from Jonah's implementation
in oxView. Likely a lot here that is not used, but it works!
https://github.com/sulcgroup/oxdna-viewer/blob/master/ts/file_handling/pdb_worker.ts
*/

async function loadPDBFromId(id) {
    return await loadPDBFromURL(`https://files.rcsb.org/download/${id}.pdb`);
}
...
async function loadPDBFromURL(url) {
    ...
    return loadPDB(pdbText, 0);
}

function loadPDB(pdbText, pdbFileInfoIndx) {
    const pdbLines = pdbText.split(/[\n]+/g);
    let pdata = pdb_step1(pdbLines);
    const strands = addPDBToScene(pdata, pdbFileInfoIndx);
    return strands;
};
...
function addPDBToScene(pdbinfo, pindx) {
    ...
    let CalcInfoNC = (res) => {
        ...
        for (let types of ring_poss) {
            ...
            if (p === undefined || q === undefined || r === undefined) {
                if (p === undefined) {
                    notify('Atom ' + types[0] + " not found in residue " + res.pdbResIdent + " in chain " + res.chainID, type = 'alert');
                }
                if (q === undefined) {
                    notify('Atom ' + types[1] + " not found in residue " + res.pdbResIdent + " in chain " + res.chainID, type = 'alert');
                }
                if (r === undefined) {
                    notify('Atom ' + types[0] + " not found in residue " + res.pdbResIdent + " in chain " + res.chainID, type = 'alert');
                }
            ...
            if (currentStrand.length === 0) {
                ...
                initlist.repeatIDs.forEach((rid, indx) => {
                    ...
                    if (nstrand.chainID.includes(rid)) { // Repeat same chain
                        let repeatStrand = [];
                        sys.push(repeatStrand);
                        strandtype.push("pro");
                        currentStrand.forEach((mon, mid) => {
    ...
    //return [
    //    strandInit, strandtype, com, [label, bFactors, xdata, "bfactor", "A_sqr"], [xdim, ydim, zdim],
    //    pdbindices, [pdbinfo.pdbfilename, pdbinfo.pdbsysinfo, pdbinfo.initlist, pdbinfo.disulphideBonds]
    //];

    // Add residue position and atom positions to the same data structure
    // We don't care about the rest for now
    for (let i=0; i<pdbinfo.pdbsysinfo.length; i++) {
        const chain = pdbinfo.pdbsysinfo[i];
        for (let j=0; j<chain.residues.length; j++) {
            const p = new THREE.Vector3().fromArray(strandInit[i][j][2]);
            chain.residues[j].position = p;
        }
    }
    return pdbinfo.pdbsysinfo;
}

export {loadPDB, loadPDBFromId, loadPDBFromURL}